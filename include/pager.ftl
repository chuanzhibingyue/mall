...
<#macro pageShow totalPages pageNumber _pageName pageSize=15 _styleClassName="gray" _maxShowNums=10 _defaultPageParameter="p" >
    <#-- 本段默认起始页 -->
    ...
    <#-- 分页的最后一页 -->
    <#local _lastPageNo = totalPages>
    <#-- 前一页号码 -->
    <#local _prePageNo = pageNumber-1 />
    <#-- 后一页号码 -->
    <#local _nextPageNo = pageNumber+1 />
     
    <#if pageNumber == _pageStartNo>
        <#local _prePageNo = _pageStartNo />
    </#if>
     
    <#if pageNumber == _lastPageNo>
        <#local _nextPageNo = _lastPageNo />
    </#if>
     
    <#if pageNumber gt _maxShowNums>
        <#local _thisPageSegment = ((pageNumber-1) / _maxShowNums)?int + 1/>
        ...
        <#if _thisPageEndNo gt totalPages>
            <#assign _thisPageEndNo = totalPages>
        </#if>
    <#else>
    ...
    <#local _htmlNoLinkLine>
        ...
        <span>共${totalPages?default(0)}页</span>
    </#local>
